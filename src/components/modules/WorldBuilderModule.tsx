...
export function WorldBuilderModule({ seriesId }: { seriesId: string | null }) {
  ...
  const hasGeminiKey = useBackendStatusStore((s) => s.services.gemini);
  ...
  const generateIdeas = async () => {
    if (!hasGeminiKey) return addToast('AI service is temporarily unavailable. Please try again shortly.', 'error');
  ...
  const saveLoreVault = async () => {
    setSavingLore(true);
    try {
      await saveSettingMut.mutateAsync({ key: 'lore_vault', value: { text: loreVault } });
      addToast('Lore vault saved', 'success');
    } catch (err) {
      addToast(`Failed to save lore vault: ${err instanceof Error ? err.message : 'Unknown'}`, 'error');
    } finally {
      setSavingLore(false);
    }
  };
          ...
          <Panel title="World Lore Continuity Vault" icon={<BookOpen size={15} />}>
            <div className="p-4 space-y-4">
              <p className="text-xs text-ink-300">
                Document world rules, running jokes, and continuity details. Referenced when generating future scripts to keep the series consistent.
              </p>
              <textarea
                value={loreVault}
                onChange={(e) => setLoreVault(e.target.value)}
                rows={12}
                placeholder="e.g. The city of Aravel never sleeps. Kael's left eye glows when he lies. Running joke: nobody can pronounce the baker's name..."
              ...
              />
              <MotionButton onClick={saveLoreVault} disabled={savingLore} className="btn-primary">
                {savingLore ? <Spinner size={14} /> : <Check size={15} />} {savingLore ? 'Saving...' : 'Save Lore Vault'}
              </MotionButton>
            </div>
          </Panel>
        </motion.div>
