...
export function StudioModule({ seriesId }: { seriesId: string | null }) {
  ...
  const fetchMetadata = useCallback(async () => {
    if (!backendStatus.gemini) {
      addToast('AI service is temporarily unavailable. Please try again shortly.', 'error');
      return;
    }
    if (!currentScript) {
              ...
              <div className="p-4 space-y-4">
                {!backendStatus.gemini && (
                  <div className="p-3 rounded-xl bg-danger-dim border border-danger/20 flex items-start gap-2">
                    ...
                    <p className="text-xs text-danger">AI Engine temporarily unavailable. Please try again shortly.</p>
                  </div>
                )}
